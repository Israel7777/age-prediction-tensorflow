...
def train(args):
    ...
    # setup model
    # base_model = InceptionV3(input_tensor=Input(shape=(IM_HEIGHT, IM_WIDTH, 3)), weights='imagenet',
    #                    include_top=False)  # include_top=False excludes final FC layer

    # base_model = VGGFace(input_tensor=Input(shape=(IM_HEIGHT, IM_WIDTH, 3)), include_top=False, weights='vggface')
    base_model = VGGFace(input_tensor=Input(shape=(IM_HEIGHT,IM_WIDTH,3)),include_top=False,weights='vggface')
...
if __name__ == "__main__":
    ...
    a.add_argument("--output_model_file", default="model/3_vggface-ft_gender.h5")
